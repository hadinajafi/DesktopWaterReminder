...
public class MainAnchorPaneLayoutController implements Initializable {
    ...
    @FXML
    private Label timerCounterLabel; //show counting down the current period till next notification. Timer "counter" used for this
    @FXML
    ...
    void saveApplyBtnAction(ActionEvent event) {
        if (timeIntervalTextField.getEditor().getText() == null
                || timeIntervalTextField.getEditor().getText().equals("")
                || timeIntervalTextField.getEditor().getText().equals("0"))
            timeIntervalTextField.getEditor().setText("60");

        userData.replace("timerPeriod", Integer.parseInt(
                timeIntervalTextField.getEditor().getText()));
    ...
    void skipBtnAction(ActionEvent event) {
        Logger.getLogger(skipButton.getClass().getName()).log(Level.WARNING, null, event);
    ...
    void glassVolSaveBtn(ActionEvent event) {
        Logger.getLogger(glassVolBtn.getClass().getName())
                .log(Level.WARNING, null, event);
        if (glassVolumeSpinner.getEditor().getText() == null
                || glassVolumeSpinner.getEditor().getText().equals("")
                || glassVolumeSpinner.getEditor().getText().equals("0"))
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        tabPane.tabMinWidthProperty().bind(root.widthProperty()
                .divide(tabPane.getTabs().size()).subtract(25));
        statPieChart.setTitle("How much you Drunk water today");
        //initializing the spinners value factory
        spinnersValueFactory();
        //load default or saved value to the spinner
        timeIntervalTextField.getEditor().setText(String.valueOf(loadTimer()));
    ...
    private void loadingChartData() {
        ...
        //checking for handle data missing: null pointer exception happened.
        if (userData.containsKey(drinkDate.getToday()))
            drinkTimes = userData.get(drinkDate.getToday());
        //add the default (0) value to the userData map and save it.
        else {
            userData.put(drinkDate.getToday(), drinkTimes);
            new FileStreams().setData(userData);
        }
        //calculate the drunk glass volume by: 2 litres is default value for every person needs to
        // drink water each day. glass value is CC and divided by 1000 to convert to litres.
        double drunkPerGlass = 2 / (userData.get("glassVol") / 1000.0);
        //drunk water is drunk times divided by drinks per glass.
        double drunkWater = drinkTimes / drunkPerGlass;
        //if data is not more than 100% percentage.
        // because maybe someone drunk water more than 2 litres.
        if (drunkWater <= 1.0) {
            ObservableList<Data> chartData = FXCollections
                    .observableArrayList(new PieChart.Data("Water Drunken", drunkWater),
                            new PieChart.Data("Didn't Drink yet!", 1.00 - drunkWater));
            statPieChart.setData(chartData);
        }
        else {
            ObservableList<Data> chartData = FXCollections
                    .observableArrayList(new PieChart.Data("Water Drunken", 1.0));
            statPieChart.setData(chartData);
        }
    }
    ...
    private void spinnersValueFactory() {
        ...
        /*
         * Setting the value factory for the Spinner, increment and decrement
         * works well and the minimum range is 0.
         */
        timeIntervalTextField.setValueFactory(new SpinnerValueFactory<Integer>() {
            ...
            public void decrement(int steps) {
                if (!"0".equals(timeIntervalTextField.getEditor().getText()))
                    timeIntervalTextField.getEditor()
                            .setText(String.valueOf(Integer.parseInt(timeIntervalTextField
                                    .getEditor().getText()) - 1));
            }

            @Override
            public void increment(int steps) {
                timeIntervalTextField.getEditor().setText(String.valueOf(Integer
                        .parseInt(timeIntervalTextField.getEditor().getText()) + 1));
            }
        });
        ...
        glassVolumeSpinner.setValueFactory(new SpinnerValueFactory<Integer>() {
            @Override
            public void decrement(int steps) {
                glassVolumeSpinner.getEditor().setText(String.valueOf(Integer
                        .parseInt(glassVolumeSpinner.getEditor().getText()) - 1));
            }
     ...
     * method used to load the user data.
    ...
    */
        ...
        counter.scheduleAtFixedRate(new TimerTask() {
            ...
            public void run() {
                Platform.runLater(() -> {   //running the FX thread
                    ...
                    timerCounterLabel.setText(hourString + ":" + minString + ":" + secString);
            ...
            //timer will start without delay and will count down every second (1000 milisec).
        }, 0, 1000);
    }

    /**
     * Load the Timer period from the file
    ...
    private int loadTimer() {
        ...
        //if the user have no saved data, we use 60 minutes as default.
        if (!userData.containsKey("timerPeriod")) {
            ...
            glassVolumeSpinner.getEditor().setText(String.valueOf(userData.get("glassVol")));
            //if the saved file and data exists, the saved data will return.
            return userData.get("timerPeriod");
        }
    }
     ...
     * Platform.runLater() used to perform this action on a FX thread.
     * otherwise the timer thread error occurs:
     * The Notification in package controlsfx can't run on a timer thread.
     * needs FX thread.
     */
    private void displayNotification() {
        Platform.runLater(() -> {   //creating the FX thread
            Notifications.create().title("Eat & Drink Healthy").text("It's Time to drink Water!")
                    .graphic(new ImageView(new Image(getClass()
                            .getResourceAsStream("/icons/water2.png"))))
    ...
    private void increaseWaterDrinkTimes() {
        ...
        //if any notifications showed to the user, so there is data
        if (userData.containsKey(drinkDate.getToday()))
            userData.replace(drinkDate.getToday(), drinkTimes);
        //unless if user haven't any data about today drink times.
        else
            userData.put(drinkDate.getToday(), 1); //1 because it is the first drink water
